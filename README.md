...
[이미지 삽입 추천 1: 교육 포스터 또는 수강 화면 캡처]
→ 글 초반에 넣으면 어떤 교육에 참여했는지 바로 보입니다.

## 가장 크게 배운 점: 네트워크를 ‘설정값’이 아니라 ‘흐름’으로 이해하기
...
[이미지 삽입 추천 2: VPC, public_subnet, private_subnet이 보이는 네트워크 구성 화면]
→ 이 문단 바로 아래에 넣으면 `/16`, `/24`, Public/Private Subnet 설명과 잘 연결됩니다.
...
[이미지 삽입 추천 3: 웹서버 접속 성공 화면 또는 Image 생성 화면]
→ Instance와 Image 설명 뒤에 넣으면 “직접 만든 서버를 이미지화했다”는 흐름이 보입니다.
...
[이미지 삽입 추천 4: Load Balancer 구성 화면 또는 Auto Scale 인스턴스 목록 화면]
→ Load Balancer, Auto Scale 설명 뒤에 넣으면 확장 구조를 보여주기 좋습니다.

[이미지 삽입 추천 5: 웹페이지에서 현재 웹 서버 IP가 바뀌는 화면]
→ “트래픽 분산을 직접 확인했다”는 부분에 가장 잘 맞는 캡처입니다.
...
[이미지 삽입 추천 6: RDS 접속 정보 입력 화면 또는 DB 샘플 데이터 조회 성공 화면]
→ RDS 실습 결과를 보여주기 좋습니다. 개인정보나 접속 정보는 가려서 올리는 것이 좋습니다.
...
[이미지 삽입 추천 7: Object Storage 컨테이너 생성 화면 또는 정적 웹사이트 접속 성공 화면]
→ Object Storage가 단순 저장소가 아니라 정적 웹페이지 제공에도 활용될 수 있음을 보여줍니다.
...
[이미지 삽입 추천 8: 리소스 삭제 체크리스트 또는 삭제 완료 화면]
→ 비용 절감과 리소스 관리에 대한 문단 뒤에 넣으면 좋습니다.
